  ...
  try {
    ...
    (async () => {
      try {
        ...
        const seeker = await storage.getUser(updated.seekerId);
        if (seeker) {
          ...
          await sendTemplatedEmailByEvent("verification_decision_seeker", (seeker as any).email, {
            first_name: (seeker as any).firstName || (seeker as any).email,
