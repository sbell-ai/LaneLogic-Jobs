...
- **Admin Bulk Job Import**: Allows administrators to upload job listings via CSV, supporting upserts, row-level validation, and error reporting. Required CSV columns: `externalJobKey`, `title`, `description`, `requirements`. Skills/keywords stored as `rawSkills`/`rawKeywords` arrays in `job_metadata` JSONB. Tagging architecture stub (`server/taggingValidator.ts`) is disabled by default (`TAGGING_ARCH_ENABLED=false`); when enabled, validates keywords against canonical tags and rejects ambiguous mappings.

### Feature Specifications
- **Job Listings**: Comprehensive job posts with internal/external apply options.
