...
type InboxItem = EmployerVerificationRequest & {
  employerName: string | null;
  employerEmail: string;
  evidence: EmployerEvidenceItem[];
};

const SOURCE_LABELS: Record<string, string> = {
  dot_number: "DOT / MC Number",
...
function DecisionPanel({ item, onComplete }: { item: InboxItem; onComplete: () => void }) {
  ...
  const decisionMutation = useMutation({
    mutationFn: (decision: "verified" | "rejected" | "needs_more") =>
      apiRequest("POST", "/api/admin/employer-verification/request/decision", {
        requestId: item.id,
        decision,
    ...
    <div className="border-t pt-4 mt-4 space-y-3">
      ...
      <div className="flex gap-2">
        ...
        <Button
          ...
          data-testid={`button-needs-more-${item.id}`}
        >
...
export default function VerificationInbox() {
  ...
  const { data: items, isLoading } = useQuery<InboxItem[]>({
    queryKey: ["/api/admin/employer-verification/inbox"],
  });

  const handleComplete = () => {
    queryClient.invalidateQueries({ queryKey: ["/api/admin/employer-verification/inbox"] });
    setExpanded(null);
  };
  ...
  return (
    <div>
      <div className="flex items-center gap-3 mb-6">
        ...
        <h2 className="text-2xl font-bold font-display" data-testid="text-verification-inbox-heading">Verification Requests</h2>
      </div>
      ...
      {!isLoading && (!items || items.length === 0) && (
        <Card className="p-8 text-center" data-testid="card-empty-inbox">
      ...
      {!isLoading && items && items.length > 0 && (
        <div className="space-y-4">
          {items.map((item) => (
            <Card key={item.id} className="p-5" data-testid={`card-verification-request-${item.id}`}>
              <div
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="font-semibold">{item.employerName || item.employerEmail}</p>
                    <p className="text-sm text-muted-foreground">{item.employerEmail}</p>
                  </div>
                </div>
                <div className="flex items-center gap-3">
                ...
                <div className="mt-4 space-y-3">
                  <div>
                    ...
                    {item.evidence.map((ev) => (
                      <div key={ev.id} className="border rounded-lg p-3 mb-2" data-testid={`card-admin-evidence-${ev.id}`}>
